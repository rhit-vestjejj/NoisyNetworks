"""
Generate all comparison plots for the Asterix DQN / DDQN runs.

For each of the 2 implementations produces 3 plots:
  1. <impl>_baseline.png  — vanilla only
  2. <impl>_noisy.png     — NoisyNet only
  3. <impl>_comparison.png — noisy vs vanilla on the same axes

Plus 2 summary plots:
  4. summary_noisy.png    — DQN-Noisy vs DDQN-Noisy
  5. summary_baseline.png — DQN-Vanilla vs DDQN-Vanilla

Run after all 4 jobs finish:
...
for impl, data in loaded.items():
    ...
    ax.set_title(f"Vanilla {label} — Asterix")
    ax.legend()
    ...
    ax.set_xlabel("env step"); ax.set_ylabel("mean reward (last 100 eps)")
    ax.set_title(f"NoisyNet-{label} — Asterix")
    ax.legend()
    ...
    _plot(ax, base_steps,  base_rewards,  f"Vanilla {label}",  BASELINE_COLOR)
    ax.set_xlabel("env step"); ax.set_ylabel("mean reward (last 100 eps)")
    ax.set_title(f"NoisyNet vs Vanilla — {label} — Asterix")
    ax.legend()
