"""
Dueling DQN on CartPole-v1, in both noisy and vanilla flavours.

  --noisy     (default): NoisyNet-Dueling, paper Eq. 15-16 with double-DQN target.
              No epsilon — exploration is the weight noise.
  --no-noisy            : vanilla Dueling (Wang 2016) with double-DQN target
              and epsilon-greedy. Linear epsilon decay.
"""
...
from model import NoisyDuelingMLP, iter_sigma
...
def train(args):
    ...
    online = NoisyDuelingMLP(obs_dim, n_actions, noisy=args.noisy).to(device)
    target = NoisyDuelingMLP(obs_dim, n_actions, noisy=args.noisy).to(device)
    target.load_state_dict(online.state_dict())
    optim = torch.optim.Adam(online.parameters(), lr=args.lr)
    buf = ReplayBuffer(args.buffer_capacity)

    rewards, sig_steps, sig_layers = [], [], []
    ...
    for step in range(1, args.total_steps + 1):
        ...
        # ---------- learn (double-DQN target; paper Eq. 4-5, 15-16) ----------
        if len(buf) >= max(args.batch_size, args.learning_starts):
            s, ac, rw, sn, te = buf.sample(args.batch_size, device)

            with torch.no_grad():
                # b* = argmax_b Q_online(y, b)  (action selected by online net)
                online.reset_noise()
            ...
            # Online sample for the actual gradient update (must be last).
            online.reset_noise()
        ...
        if step % 200 == 0:
            ...
            sig_layers.append(list(iter_sigma(online)))
        if step % 2000 == 0:
            recent = rewards[-20:] if rewards else [0.0]
            extra = f" | sigma {sig_layers[-1]}" if args.noisy and sig_layers[-1] else ""
    ...
    title_suffix = "NoisyNet-Dueling" if args.noisy else "vanilla Dueling"
    ...
    ax.set_title(f"{title_suffix} on CartPole-v1"); ax.legend(); fig.tight_layout()
